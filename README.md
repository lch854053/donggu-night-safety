...
보안등은 API가 좌표를 공개하지 않아 동구청 제공 CSV를 사용합니다. `scripts/data/donggu-security-lights.csv`를 최신 자료로 교체한 뒤 위 명령으로 반영하세요. 자료가 매년 말 기준이라 매년 1월 15일에 GitHub Actions가 갱신 알림 이슈를 자동 생성합니다(`.github/workflows/security-light-refresh-reminder.yml`). 가로등은 공공데이터포털 동구 가로등현황(2024-04-15) API를 서버 측에서 수집합니다. 고유 관리번호 3,805건을 suffix를 제거한 264개 관리그룹으로 묶습니다. 각 행은 고유 관리대상이지만 개별 등주 좌표는 공개되지 않고 그룹 대표좌표만 공유하므로, 이를 Point 광원으로 표시하거나 계산하지 않습니다. 도로명·대표좌표·행정동으로 가까운 도로에 anchor를 정하고, 실제 중심선에서 연결된 양방향 총 200m(지번만 있을 때 최대 150m) corridor의 설치 근거를 추정합니다. 지도 패널에서 기본 꺼짐인 **가로등 추정구간**을 켜면 원본 도로 중심선의 연결 구간만 선으로 표시합니다(실선/긴 점선/짧은 점선 = 높음/보통/낮음). 관리대상 수는 도로 속성으로 보존하되 조명 강도에 직접 가산하지 않습니다. 매칭·제외 통계는 `public/data/road-light-evidence.json`, 지도용 도형은 `public/data/road-light-corridors.geojson`, 모델 비교 및 이상치 목록은 `public/data/road-light-analysis.json`에 있습니다. 키가 없으면 월간 전체 갱신 시 기존 추정 데이터를 보존하며, 가로등만 갱신할 때는 실패합니다.
...
| 차원 | 하위 지표 | 현재 데이터 |
| --- | --- | --- |
| 조명·가시성 (25%) | 보안등 실좌표 점수(커버리지 50% · 최대 암구간 30% · 균일도 20%)와 독립된 가로등 corridor 추정 점수를 포화 결합 | 보안등 개별 좌표 ✅ · 가로등 도로구간 추정 ✅ (lux 아님) |
...
- **조명**: 실제 보안등 개별 좌표를 기준으로 도로를 5m 간격으로 샘플링한다. 보안등의 거리감쇠(σ=20m·cutoff=60m)를 `1 − ∏(1 − 개별 영향)`으로 포화 결합해 보안등 커버리지(50%), 최대 연속 암구간(30%), 어두운 구간 품질(20%)에서 `securityLightingScore`를 산출한다. `actualLightingScore`는 이 점수의 호환 별칭이다. 가로등 API는 관리그룹별 대표좌표만 공개하므로 개별 Point 광원으로 쓰지 않는다. 동일 도로명·연결된 중심선 corridor의 설치 근거(0~1), 해당 도로의 **연결된** 증거구간 길이·관리그룹 수를 이용해 별도 `roadLightingScore`를 산출한다. 기본식은 `min(신뢰도별 상한, round(20 + 50 × evidence + 15 × continuity + 최대 4점 도로 폭 보조))`이며 폭원 보조는 여러 그룹·충분한 연속성·도로명 신뢰도가 함께 있을 때만 허용한다. 낮은 신뢰도는 최대 55점, 보통 65점, 높은 신뢰도는 85점이다. `managedUnitCount`는 관리대상 규모이지 실제 등주 개수·위치가 아니며 점수에 직접 곱하지 않는다. 최종 `lightingScore`는 OLD 보너스·MAX·UNION을 비교해 `round(100 × [1 − (1 − security/100) × (1 − road/100)])`의 **UNION**을 적용한다. 추정 가로등으로 보안등 커버리지·최대 암구간·균일도를 변경하지 않는다. 점수는 실제 조도(lux), 실제 등주 위치나 설치 간격을 의미하지 않는 **상대적 참고값**이다.
