...
- **조명**: 실제 보안등 개별 좌표를 기준으로 도로를 5m 간격으로 샘플링한다. 보안등의 거리감쇠(σ=20m·cutoff=60m)를 `1 − ∏(1 − 개별 영향)`으로 포화 결합하여 커버리지(50%), 최대 연속 암구간(30%), 어두운 구간 품질(20%)을 산출한다. 가로등 API에서는 각 행이 고유 관리번호를 지니지만 관리그룹별 대표좌표만 알 수 있으므로 이를 개별 Point 광원으로 쓰지 않는다. 동일 도로명·연결된 중심선의 제한적 corridor에서 `roadLightingEvidence`(0~1)를 추정하며 `managedUnitCount`는 실제 등주 개수나 개별 위치가 아니라 **관리대상 규모**다. 관리대상 수에 따른 강도 포화 모델 A/B와 수를 강도에 반영하지 않는 모델 C를 비교한 뒤 C를 채택했다. 조명점수는 Model B `actualLightingScore + (100 − actualLightingScore) × roadLightingEvidence × 0.25`(반올림)를 유지한다. 지번 기반 신뢰도 0.6 근거는 절반만 반영하고 0.6 미만은 제외하며, 보안등 근거가 없으면 추정치 단독 기여로 60점을 초과하지 않는다. 최대 암구간·커버리지·어두운 구간 품질은 추정 가로등으로 변경되지 않는다. 모두 **상대적 조명환경 참고값**이며 실제 조도(lux), 실제 등주 위치나 설치 간격이 아니다.
