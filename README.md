...
보안등은 API가 좌표를 공개하지 않아 동구청 제공 CSV를 사용합니다. `scripts/data/donggu-security-lights.csv`를 최신 자료로 교체한 뒤 위 명령으로 반영하세요. 자료가 매년 말 기준이라 매년 1월 15일에 GitHub Actions가 갱신 알림 이슈를 자동 생성합니다(`.github/workflows/security-light-refresh-reminder.yml`). 가로등은 공공데이터포털 동구 가로등현황(2024-04-15) API를 서버 측에서 수집합니다. 원본 3,805개 관리행이 단 264개 대표좌표를 공유합니다. **개별 등주 위치로 사용할 수 없어** 지도에 가로등 Point를 표시하지 않고, 도로명·대표좌표·행정동으로 도로구간별 설치 근거를 추정합니다. 매칭·제외 통계는 `public/data/road-light-evidence.json`, 모델 비교 및 이상치 목록은 `public/data/road-light-analysis.json`에 있습니다. 키가 없으면 월간 전체 갱신 시 기존 도로구간 추정 근거를 보존하며, 가로등만 갱신할 때는 실패합니다.
...
| 차원 | 하위 지표 | 현재 데이터 |
| --- | --- | --- |
| 조명·가시성 (25%) | 보안등 커버리지 50% · 최대 암구간 30% · 조명 균일도 20% + 추정 가로등 근거의 제한적 보조 보정 | 보안등 개별 좌표 ✅ · 가로등 도로구간 추정 ✅ (lux 아님) |
...
- **조명**: 실제 보안등 개별 좌표를 기준으로 도로를 5m 간격으로 샘플링한다. 보안등의 거리감쇠(σ=20m·cutoff=60m)를 `1 − ∏(1 − 개별 영향)`으로 포화 결합하여 커버리지(50%), 최대 연속 암구간(30%), 어두운 구간 품질(20%)을 산출한다. 가로등 API는 다수 관리번호가 동일 대표좌표를 공유해 실제 설치위치가 확인되지 않으므로 Point 광원으로 쓰지 않는다. 대신 도로명·주소·대표좌표로 도로구간별 `roadLightingEvidence`(0~1)를 추정하며 관리행 수는 포화형 근거일 뿐 실제 가로등 개수가 아니다. 비교 검토한 Model B는 `lightingScore = actualLightingScore + (100 − actualLightingScore) × roadLightingEvidence × 0.25`(반올림)를 사용한다. 지번 기반 신뢰도 0.6 근거는 절반만 반영하고 0.6 미만은 제외하며, 보안등 근거가 없으면 추정치 단독 기여로 60점을 초과하지 않는다. 최대 암구간·커버리지·어두운 구간 품질은 추정 가로등으로 변경되지 않는다. 모두 **상대적 조명환경 참고값**이며 실제 조도(lux), 실제 등주 위치나 설치 간격이 아니다.
